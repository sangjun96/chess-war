...
- Choose **Attack**, then click an orange tile to fire the selected piece's skill without moving it. Skill targeting uses its own range instead of the piece's chess movement. For area skills, hover an orange tile to preview the red tiles that will be hit; area skills damage every enemy in their effect radius.
...
- Each piece type has a replaceable combat skill. Edit the assignments in
  ...
  Optional sounds can be added as `assets/audio/skills/<skill-id>.ogg`.
